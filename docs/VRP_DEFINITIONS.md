...
$w_t(k,\tau) = \sigma_{t,\mathrm{ann}}^2(k,\tau) \cdot \tau$

where $\sigma_{t,\mathrm{ann}}(k,\tau)$ is the **annualized** implied volatility.

For the ATM point ($k=0$) at the target horizon $\tau_t$, define:

$w^{\mathrm{ATM}}_t \equiv w_t(0,\tau_t)$

Then the **annualized implied variance** at ATM is:

$IV^2_{t,\mathrm{ann}} \equiv \sigma^2_{t,\mathrm{ann}}(0,\tau_t) = \frac{w^{\mathrm{ATM}}_t}{\tau_t}$

Equivalently, the ATM implied volatility is:

$IV_{t,\mathrm{ann}} = \sqrt{\frac{w^{\mathrm{ATM}}_t}{\tau_t}}$

**Important:** In code and reporting, "ATM IV" must be distinguished from "ATM total variance." Ideally, `atm_iv` would represent $IV_{t,\mathrm{ann}}$ (volatility). In this codebase, the variable `atm_iv` stores **total variance** $w^{\mathrm{ATM}}_t$ (SVI raw); annualized IV² is computed as `atm_iv / tau`.
...
Let $t \to t+M$ denote the forward window whose calendar span is $\mathrm{span\_days}_t \geq H$ (we choose $M$ such that the forward window spans at least $H$ calendar days; if the next $H$ calendar days include weekends/holidays, $M$ is the number of trading-day returns in that span).
...
$RV_{t,\mathrm{ann}} \equiv RV_{t,H} \times \frac{365}{\mathrm{span\_days}_t}$

This makes $RV_{t,\mathrm{ann}}$ directly comparable to $IV^2_{t,\mathrm{ann}}$, since both are annualized variances.
...
$VRP_{t,H} \equiv RV_{t,\mathrm{ann}} - IV^2_{t,\mathrm{ann}}$
